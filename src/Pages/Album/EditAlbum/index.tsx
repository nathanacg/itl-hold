...
export default function EditAlbum() {
     ...
     const params = route.params as { albumId: number, albumTitle: string, albumImage: string }
     ...
     useEffect(() => {
          console.log('----> ', params.albumImage)
     }, [params.albumImage])

     const editAlbum = async () => {
          const title = editTitle;
          const form = new FormData();
          if (title == params.albumTitle) {
               form.append('title', params.albumTitle);
          } else {
          ...
          if (albumImage) {
               ...
               form.append('file', params.albumImage);
          }
          ...
          await updateAlbum(form).then((response) => {
               navigation.navigate('Album', {
                    titleAlbum: title,
                    imagealbum: albumImage ? albumImage?.uri : params.albumImage,
                    albumId: params.albumId,
                    userId: user.userId
               });
               seteditAlbumTitle('')
               {
                    albumImage ? setCaptureImage({
                         fileName: '',
                         uri: '',
                         mime: ''
                    }) : <></>
               ...
               <Container>
                    ...
                    <ContentainerConfigurations>
                         <>
                              ...
                              <View>
                                   <TextOptionSelectedBottom>Altere sua capa</TextOptionSelectedBottom>
                                   {Object.keys(captureImage)?.length > 0 ? (
                                        <ImageAlbumContainer>
