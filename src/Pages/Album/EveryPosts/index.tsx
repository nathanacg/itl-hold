...
const EveryPosts = (props: EveryPosts) => {
     ...
     useEffect(() => {
          const filteredPosts = props.publicationsAlbums.filter(
               (post) => (post.postCategorie != 'Série' && post.postCategorie != 'Filme') && post.postCategorie
          );
                    ...
                    ListEmptyComponent={
                         <View style={{ alignItems: 'center', justifyContent: 'center', top: 20, height: 60 }}>
                              <Text style={{ color: theme.lightGray }}>
                                   Você não tem outros tipos de post
                              </Text>
                         </View>
                    }
                    renderItem={({ item }) => {
