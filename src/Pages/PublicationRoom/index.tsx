...
import { BackHandler, Dimensions, FlatList, Image, ImageStyle, PermissionsAndroid, Platform, StyleProp, TouchableOpacity, View, Modal } from 'react-native'
...
import { getMoviesList, listAbleToMarcations } from "../../Service/Publications"
...
import useCreatePost, { markerUsers } from "../../GlobalState/createPost.zustand";
...
export default function PublicationRoom() {
    ...
    const { setPost, setFiles, setImagesUpload, setMarkedPerson, setMarkerPhoto } = useCreatePost()
    ...
    const [attachmentFile, setAttachmentFile] = useState<DocumentPickerResponse[]>()

    const [metrics, setMetrics] = useState<string>('')

    const [selectedPostImages, setSelectedPostImages] = useState<boolean>(true)
    const [optionAddSelected, setOptionAddSelected] = useState<"default" | "list" | "ranking">("default")
    const [openUsersModal, setOpenUsersModal] = useState(false)
    const [audioPath, setAudioPath] = useState('')

    const [link, setLink] = useState<string>('')

    const [emojiSelected, setEmojiSelected] = useState<{ name: string, imageUrl: any, selectedImage: any }>()

    const [movieSelected, setMovieSelected] = useState<{ id: number, name: string, description: string, image?: string, tmdbMovieId: number | string | null }>()
    const [seriesSelected, setSeriesSelected] = useState<{ name: string, description: string, image?: string, tmdbMovieId: number }>()
    const [bookSelected, setBookSelected] = useState<{ name: string, description: string, image?: string, tmdbMovieId: number | string }>()
    const [musicSelected, setMusicSelected] = useState<{ name: string, description: string, image?: string, tmdbMovieId: number | string }>()
    const [articleSelected, setArticleSelected] = useState<{ name: string, description: string, image?: string, tmdbMovieId: number }>()
    const [podcastSelected, setPodcastSelected] = useState<{ name: string, description: string, image?: string, tmdbMovieId: number }>()
    ...
    const [markerPhoto, setMarkerPhotoImage] = useState<{ image?: string, persons: number[] }[]>([])
    const [imageMarker, setImageMarker] = useState<markerUsers[]>([])



    const [legendButtons, setLegendButtons] = useState<{ letter: string, text: string }[]>([])
    ...
    const categorys: Category[] = [
        { name: "Filme", imgSource: require("../../Assets/Icons/cameraMovie.png"), selectedImage: require("../../Assets/Icons/cameraMovieWhite.png"), style: { width: 20, height: 20, resizeMode: "contain", marginTop: 2, marginLeft: 2 } },
        { name: "Série", imgSource: require("../../Assets/Icons/clapperboard.png"), selectedImage: require("../../Assets/Icons/clapperboardWhite.png"), style: { width: 20, height: 20, resizeMode: "contain", marginTop: 2 } },
        { name: "Livro", imgSource: require("../../Assets/Icons/openBook.png"), selectedImage: require("../../Assets/Icons/openBookWhite.png"), style: { width: 20, height: 20, resizeMode: "contain", marginTop: 2 } },
        { name: "Música", imgSource: require("../../Assets/Icons/music.png"), selectedImage: require("../../Assets/Icons/musicWhite.png"), style: { width: 20, height: 20, resizeMode: "contain", marginTop: 4, marginRight: 2 } },
        { name: "Artigo", imgSource: require("../../Assets/Icons/article.png"), selectedImage: require("../../Assets/Icons/articleWhite.png"), style: { width: 20, height: 20, resizeMode: "contain", marginTop: 3 } },
        { name: "Podcast", imgSource: require("../../Assets/Icons/podcastMicrophone.png"), selectedImage: require("../../Assets/Icons/podcastMicrophoneWhite.png"), style: { width: 20, height: 20, resizeMode: "contain", marginTop: 2 } },
    ]
    ...
    useEffect(() => {
        if (inputSearch.length > 3 && categorySelected == "Série") {
            (async () => {
                try {
                    const response: ITvSeries[] = await getBySeries(encodeURI(inputSearch), 'w300')
    ...
    useEffect(() => {
        if (inputSearch.length > 3 && categorySelected == "Filme") {
            (async () => {
                await getMoviesList(encodeURI(inputSearch)).then(res => {
                    const filterResults = res.data.movies.filter(movie => movie.movieImagens?.poster_sizes?.w92.length > 0)
    ...
    useEffect(() => {
        if (inputSearch.length > 3 && categorySelected == "Música") {
            (async () => {
                const response: ItemMusic[] = await getBySearchMusic(inputSearch, token)
                setSeacrMusicList(response)
            })()
        }
    }, [inputSearch])

    const [images, setImages] = useState<{ key: string, fileName: string, url: string, markedUsers: { name: string; address: string, positionX: number, positionY: number }[] }[]>([])

    useEffect(() => {
        const setImagesOnCapture = () => {
            captureImage.map((item: { uri?: any; filename: any; }, index: number) => {
                setImages([...images, {
                    key: `${index + 1}`,
                    url: item.uri || '',
            ...
            setImages(captureImage.map((item, index) => (
                {
                    key: `${index + 1}`,
                    url: item.uri || '',
                    fileName: item.filename,
    ...
    const removeUser = (item: any) => (
        setImages(images.map((img, index) => (
            index == currentItemIndex ? { ...img, markedUsers: img.markedUsers?.filter(user => user.address != item.address) } : img
        )))
    )
    ...
    const savePreview = async () => {
        ...
        for (var i = 0; i < captureImage?.length; i++) {
            data.append("file", {
                uri: captureImage[i].uri,
                type: mime.getType(captureImage[i].uri || ''),
                name: captureImage[i].filename + ".jpg",
            })
        }

        if (attachmentFile) {
            data.append('file', {
                uri: attachmentFile[0].uri,
        ...
        navigation.push('PostPreview', { audioPath: audioPath, metrics: metrics, attachmentFile: attachmentFile, link: link })
    }
    ...
    const handleCheckBoxChangePerson = (profileImage: string, Id: number, userNickname: string, name: string) => {
        setUsersSelectedPerson(prevUsersSelected => {
            const userIndex = prevUsersSelected.findIndex(user =>
    ...
    const handleRemoveUser = (item: UserProfile, marker?: string) => {
        setUsersSelectedPerson(prevUsersSelected => {
            if (!marker) {
                const updatedUsers = prevUsersSelected.filter(user =>
                    item.userId !== user.id
                );
                setMarkedPerson([])
                ...
                const findRemoveIndex = usersSelectedPerson.findIndex(user => user.userId == item.userId && user.marker == marker)

                if (findRemoveIndex == 0) {
                    const temp = usersSelectedPerson.splice(1, usersSelectedPerson.length - 1)
                    setMarkedPerson([]);
                    return temp
                } else {
                    const temp = usersSelectedPerson.slice(0, findRemoveIndex)
                    const temppzin = usersSelectedPerson.slice(temp.length + 1, usersSelectedPerson.length)
    ...
    useEffect(() => {
        ...
        if (temp && !!temp.uri) {
            ...
            let menorizeMarker = {
                image: temp?.uri,
                persons: usersSelected.length > 0 ? usersSelected.splice(0, usersSelected.length) : usersSelected,
            }
            setMarkerPhotoImage(oldMenorize => [
                ...oldMenorize,
                menorizeMarker
            ])
        }
    ...
    useEffect(() => {
        if (markerPhoto) {
            const groupedPhotos = markerPhoto.reduce((accumulator, photo) => {
                ...
                const existingImageIndex = accumulator.findIndex(item => item.image === image);

                if (existingImageIndex !== -1) {
                    const existingPersonsIndex = accumulator[existingImageIndex].persons.findIndex(existingPerson => existingPerson === persons[0]);
            ...
            <FlatList
                ...
                renderItem={({ item }) => (
                    <ProfileResultFinders
                        ...
                        onCheckBoxChangePerson={() => handleCheckBoxChangePerson(item.profileImage, item.userId, item.userNickname, item.userName)}
                        id={item.userId}
    ...
    function transformarMatriz(matrizOriginal: UserProfile[]): { [key: string]: UserProfile[] } {
                ...
                <MainContainer>
                            ...
                            return (
                                <CategoryCard key={item.name + index}>
                                    <OptionContainer
                                        ...
                                        style={isSelected ? { backgroundColor: theme.primarycolor } : { borderWidth: 1 }}>
                                        <Image style={item.style} source={isSelected ? item.selectedImage : item.imgSource} />
                                    </OptionContainer>
                    ...
                    {attachmentFile?.[0] && (
                        <DocContainer marginTop marginBottom>
                            <DocumentCard size="95%" absolute={false} scale={1} docInfos={{ name: attachmentFile[0].name, url: attachmentFile[0].uri, size: attachmentFile[0].size }} position={{ x: 0, y: 0 }} storyDocument={{
                                url: "",
                                name: ""
                            }} />
                            <Ionicons
                    ...
                    <AddList>
                        <TouchableOpacity onPress={() => navigation.push("CameraPub", {
                            nextGaleryRouteName: "Publication",
                            routeParams: {}
                        })} >
                            <Image style={{ width: 30, height: 30, resizeMode: 'contain' }} source={require("../../Assets/Icons/cameraGrey.png")} />
                        </TouchableOpacity>

                        <TouchableOpacity onPress={() => navigation.push("PubliGalery", {
                            nextGaleryRouteName: "Publication",
                            routeParams: {}
                        })}>
                            <Image style={{ width: 30, height: 30, resizeMode: 'contain' }} source={require("../../Assets/Icons/galeryGrey.png")} />
                        </TouchableOpacity>

                        <TouchableOpacity onPress={() => openFileExplorer()}>
                            <Image style={{ width: 25, height: 25, resizeMode: "contain", marginRight: -7, marginLeft: -7 }} source={require("../../Assets/Icons/clip.png")} />
                        </TouchableOpacity>
                        ...
                        {movieSelected && (
                            <TouchableOpacity onPress={() => handleOptionsAdd("ranking")}>
                                <Image style={{ width: 23, height: 23, resizeMode: "contain" }} source={optionAddSelected == "ranking" ? require("../../Assets/Icons/faceRankingBlue.png") : require("../../Assets/Icons/faceRanking.png")} />
                            </TouchableOpacity>
                        )}
                        {images.length < 1 && (
                            <TouchableOpacity onPress={() => {
                                ...
                                <Image style={{ width: 23, height: 23, resizeMode: "contain" }} source={require("../../Assets/Icons/userGrey.png")} />
                            </TouchableOpacity>
                        )}
                    </AddList>
                    {
                        optionAddSelected == "list" ? <AddButtons buttons={legendButtons} setButtons={setLegendButtons} /> :
                            optionAddSelected == "ranking" ? <SetAvaliation selectedEmoji={emojiSelected} setSelectedEmoji={setEmojiSelected} /> :
                                optionAddSelected == "default" && selectedPostImages ?
                                    (<>
                                        <ScrollView
                                            horizontal
                                        >
                                                    ...
                                                    renderItem={({ drag, item }) => (
                                                        <ScaleDecorator >
                                                            <View style={{ flexDirection: 'column' }}>
                                                                <PostImage
                                                                    onSelectMore={() => navigation.navigate("PubliGalery", {
                                                                        nextGaleryRouteName: "CustomPhotoEditor",
                                                                        routeParams: { width: imageWidth, height: imageWidth }
                                                                    })}
                                                                    ...
                                                                    onLongPress={drag}
                                                                    setImages={setImages}
                                                                    setTagUsers={(tagUser, positionX, positionY) => {
                                                                        const newImages = images.map(img => img.key == item.key ?
                                                                            { ...img, markedUsers: img.markedUsers?.map(user => user.address == tagUser.address ? { ...tagUser, positionX, positionY } : user) } : img)
                                                                        setImages(newImages)
                                                                    }}
                                                        ...
                                                        {images.length > 0 &&
                                                            (
                                                                <View style={{ flexDirection: 'row', width: '100%' }}>
                                                                    {Object.values(transformarMatriz(usersSelectedPerson)).map((key, index) => {
                                                                        return (
                                                                            <View
                                                                                style={{
                                                                                    width: "100%",
                    ...
                    {categorySelected == 'Filme' ? (
                        <>
                            ...
                            <View style={{ flexDirection: 'row', marginTop: -15, justifyContent: 'space-between' }}>
                                <SpoilerContainer>
                                    {(categorySelected === 'Filme' || categorySelected === 'Série') &&
                                        <>
                                            <OptionContainer
                                                onPress={() => setHasSpoiler(!hasSpoiler)}
                                                style={hasSpoiler ? { backgroundColor: theme.primarycolor } : { borderWidth: 1 }}>
                                                <Image source={hasSpoiler ? require("../../Assets/Icons/spoilerWhite.png") : require("../../Assets/Icons/spoilerGray.png")} />
                                            </OptionContainer>
                                            <SpoilerText>
                                    ...
                                    <>
                                        ...
                                        <View style={{ borderRadius: 10, overflow: "hidden" }}>
                                            <FlatList
                                                ...
                                                renderItem={({ item }) => (
                                                    <SearchCard key={item.id} onPressFunction={() => {
                                                        setSeriesSelected({
                                                            name: item.name,
                                                    ...
                                                    }} category='Série' image={item?.poster_path?.replace("w300", 'w342')} name={item.name} description={item.overview} />
                                                )}
                                            />
                                        </View>
                                    </>
                                                        ...
                                                        <SearchCard
                                                            onPressFunction={() => {
                                                                setBookSelected({
                                                                    ...
                                                                    image: item.volumeInfo && item.volumeInfo.imageLinks.thumbnail ? `https://books.google.com/books/content?id=${item.id}&printsec=frontcover&img=1&zoom=4&edge=curl&imgtk=AFLRE70bs7XlaW7qtsTDwrOsMpo6d2iylD9zrybGEhpXsv9J4yOVgCKsluOJHSBAYpXaYsBy_3j4y5vIRA9Twtj_9mbXLdp9XQtHbGUEsYTTgRWVWoO1kxj5Sl1MqjAfxU55-sh24iqv&source=gbs_api` : null,
                                                                    tmdbMovieId: item.id,
                                                                });
                                                    ...
                                                    return (
                                                        ...
                                                        <SearchCard onPressFunction={() => {
                                                            setMusicSelected({
                                                                ...
                                                                image: item.album.images && item.album?.images[0] ? item.album?.images[0].url : '',
                                                                tmdbMovieId: item.id
                                                            })
                                                        ...
                                                        }} category='Música' image={item.album?.images && item.album?.images[0]?.url ? item.album?.images[0].url : ''} name={item.name} description={item.artists[0].name} />
                                                    )
                                                }}
                                            />
                                        </View>
                    ...
                    {categorySelected == 'Artigo' ? (
                        <>
                            <InputLink placeholder='Digite aqui o seu link' value={link} onSetText={setLink} />
                    ...
                    >
                        ...
                        <SafeAreaViewContainer>
                            ...
                            {menoUsers}
                            <Button typebutton="blue" textButton="Concluir" pressFunction={() => {
                                setMarker(undefined)
                    ...
                    <BottomModal
                        ...
                        children={
                            <>
                                ...
                                <Button typebutton="blue" textButton="Concluir" pressFunction={() => { handleModalOpen(); }} />
