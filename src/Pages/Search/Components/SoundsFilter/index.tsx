...
export default function SoundsFilter(props: PubliCard) {
    const [publi, setPubli] = useState<Post[]>([]);
    const { setPost } = useOtherProfilePost();
    const navigation = useNavigation<StackRoutes>();

    const filteredPubli = publi.filter(item => item.postCategorie === 'Música');
        ...
        <Container>
            <ContainerGroups>
                <FlatList
                    ...
                    numColumns={3}
                    scrollEnabled={false}
                    keyExtractor={(item, index) => 'post' + index + item.postHexId}
                    renderItem={({ item }) => {
